<template>
  <v-container 
    style="font-family: 'Nunito', sans-serif" class="px-md-16">
    ...
    <div class="mt-4">
      <v-row>
        <v-col cols="12" sm="3">
          <v-menu offset-y>
            <template v-slot:activator="{ on, attrs }">
              <v-btn v-bind="attrs" v-on="on" depressed color="white" large class="black--text text-subtitle-1" rounded block>
                ...
                <v-avatar size="30" class="ml-4">
                  <v-img v-if="selectedLocation === 'clientCurrentState'" :src="require('@/assets/img/my-location.png')"
                    contain>
                  </v-img>
                  <v-img v-if="selectedLocation === 'Worldwide'" :src="require('@/assets/img/world.png')" contain>
                  </v-img>
        ...
        <v-col cols="12" sm="2">
          <v-menu offset-y>
            <template v-slot:activator="{ on, attrs }">
              <v-btn v-bind="attrs" v-on="on" depressed large color="white" rounded block>
                <span class="black--text text-subtitle-1">
                  {{ selectedClub }}
                </span>
                <v-avatar size="50" class="ml-4">
                  <v-img :src="require('@/assets/img/fish-facing-left.png')" contain></v-img>
                </v-avatar>
              </v-btn>
            </template>
            <v-list>
              <v-list-item-group v-model="selectedClub" color="primary">
                <v-list-item v-for="(item, i) in clubs" :key="i" v-bind:value="item.name">
                  <v-list-item-content>
        ...
        <v-col cols="12" sm="7">
              <form class="nosubmit">
                <input class="nosubmit" v-model="searchPostedIdea" type="search" placeholder="Search...">
              </form>
          </v-col>
      </v-row>
    </div>
    ...
    <v-card class="mt-4 py-2 px-4 rounded-xl" style="cursor: pointer" v-for="item in postedIdeas.customer_pitches" :key="item.id"
      flat>
      <v-row class="d-flex align-center">
        <v-col cols="2" sm="1">
          <v-avatar>
          ...
          <div class="ml-3 ml-md-0">
            <div class="title-text font-weight-light d-flex align-center">
              ...
              <div>
                <v-tooltip right color="#000" content-class="right-tooltip">
                  <template v-slot:activator="{ on, attrs }">
                    <v-img width="40px" height="34px" src="https://i.ibb.co/0jxSZt0/fish-facing-left.png" contain
                      v-bind="attrs" v-on="on"></v-img>
                  </template>
                  <span v-if="item.customer.has_fish == 'BIG'"> Big Fish </span>
                  <span v-if="item.customer.has_fish == 'SMALL'"> Small Fish </span>
                  <span v-if="item.customer.has_fish == 'MEDIUM'"> Medium Fish </span>
                </v-tooltip>
              </div>
            </div>
            <div class="d-flex">
              <p class="text-caption mb-0">My location:</p>
              <div class="custom-chip ml-2 rounded-pill px-3 font-weight-regular">
                <span class="mr-1">🌏</span>
            ...
            <div class="d-flex mt-2">
              ...
              <div v-if="item.potential_yearly_revenue.length > 1" class="custom-chip ml-2 px-3">
                💰
                <strong> {{ item.potential_yearly_revenue }}</strong>
              </div>
            </div>
        ...
        <v-col cols="8" sm="5">
          <p class="mb-1 text-caption">I'm interested in:</p>
          <v-chip class="mb-1" color="#FAEC03" small style="border: 1px solid black">
            Partnership with Ecommerce shop
          </v-chip>
          <v-chip color="#FAEC03" small style="border: 1px solid black">
            Partnership with Ecommerce shop
        ...
        <v-col cols="2" sm="1">
          <a style="text-decoration: none;" :href="'mailto:' + item.customer.email">
            <v-btn fab depressed color="#1C4570" dark>
              <v-icon> mdi-email </v-icon>
            </v-btn>
          </a>
...
export default {
  ...
  data() {
    return {
        ...
        searchClubData: [
            ...
            locations: 'Chicago + United States of America(USA)',
            budget: '$10k - 30k',
            img: '/avatar.png',
            interestedIn: [
                { interest: 'Partnership with ECommerce shop' },
            ...
            locations: 'Atlanta + United States of America(USA)',
            budget: '$40k - 80k',
            img: '/avatar.png',
            interestedIn: [
                { interest: 'Partnership with ECommerce shop' },
            ...
            locations: 'Cambridge + United Kingdom(UK)',
            budget: '$100k - 150k',
            img: '/avatar.png',
            interestedIn: [
                { interest: 'Partnership with ECommerce shop' },
  ...
  computed: {
    searchClubFilter() {
      let postedIdeaValue = this.postedIdeas.customer_pitches;
      if (this.searchPostedIdea != '' && this.searchPostedIdea) {
        postedIdeaValue = postedIdeaValue.filter((item)=>{
            item.name
            .toUpperCase()
            .includes(this.searchPostedIdea)
        })
      }
    }
  },
